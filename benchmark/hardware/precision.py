...
class StaticFP8Linear(torch.nn.Module):
    ...
    def clear_cache(self):
        """Purge cached dequantized weight tensor to reclaim VRAM."""
        self._cached_weight = None
        self._cached_dtype = None
...
def clear_fp8_linear_caches(model: torch.nn.Module):
    """Clear dequantized weight caches in all StaticFP8Linear layers to free memory."""
    for m in model.modules():
        if isinstance(m, StaticFP8Linear):
            m.clear_cache()
