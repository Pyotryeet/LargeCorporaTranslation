"""Autoregressive inference backend — production-optimized CUDA/MPS/CPU decode.

=== ACTIVE OPTIMIZATIONS (verified live, 2026-06-26) ===

MEMORY:
  • Pinned memory H2D transfers — DMA at ~6.6 GB/s (2.1× vs pageable).
  • PagedAttention KV-cache — block-level allocation (opt-in via --paged-attention).
  • Weight quantization (INT8/INT4) via bitsandbytes (opt-in, counterproductive on H200).

COMPUTE:
  • torch.compile(mode="reduce-overhead" / "default") — version-gated inductor fusion.
  • FlashAttention-2 via SDPA backend (1.17-1.23× throughput).
  • Transformer Engine FP8 — static weight-only FP8 with dequant-on-read.

I/O:
  • CUDA stream overlap — async H2D while GPU computes.
  • Pinned memory pipeline — page-locked tensors, pool reuse.
  • Lock-free thread-local tokenizers — N× tokenization throughput.

FEATURES REMOVED (architecturally broken):
  • CUDA Graph manual capture — past_key_values cannot be a static graph input.
    torch.compile internally handles graph capture correctly.
  • Custom Triton fused kernels — crashed outside inductor graph (pointer type error).
    Removed permanently. torch.compile handles kernel fusion.
  • JIT CUDA C++ kernels — source strings were None; Metal wrapper was a no-op.
  • INT8 KV-cache quantization — on H200 (141 GB) with 4B models, unnecessary.

=== ARCHITECTURE ===

  load()
    ├─ NCCL P2P + Flash SDPA enabled
    ├─ QAT / Q4_0 model dispatch (Gemma 4 family)
    ├─ torch.compile applied (version-gated: PT >= 2.12)
    ├─ TE FP8 → static FP8 (skip lm_head)
    ├─ Speculative decoder factory (opt-in)
    └─ PagedAttention init (opt-in, flag-gated)

  translate_batch()
    ├─ Async H2D (CUDA stream overlap)
    ├─ Speculative dispatch (if active)
    ├─ PREFILL: standard forward → populate KV-cache
    └─ DECODE LOOP: per-token model() with accumulated past_key_values
"""
...
def _enable_cuda_malloc_async() -> bool:
    ...
    Uses env var approach (works on all PyTorch versions).
    ...
    try:
        ...
        # Force PyTorch to pick up the new allocator config
        if hasattr(torch.cuda, 'memory') and hasattr(torch.cuda.memory, 'CUDAPluggableAllocator'):
            try:
                ...
                pass  # PT 2.12+ deprecated the setter; env var is sufficient
        logger.info("cudaMallocAsync allocator enabled — stream-ordered, zero-fragmentation")
        return True
    except Exception as e:
        logger.warning("cudaMallocAsync not available: %s", e)
        return False
                ...
                if i != j:
                    ...
                    if can_access:
                        try:
                            ...
                            _failed_pairs.append(f"GPU{i}→GPU{j}: {e}")
        if _failed_pairs:
            logger.warning(
                "NCCL P2P: %d GPU pair(s) failed to enable peer access: %s",
                len(_failed_pairs), "; ".join(_failed_pairs),
            )
        else:
            logger.info("NCCL P2P enabled between all GPU pairs")
        return len(_failed_pairs) == 0
    except Exception as e:
        logger.warning("NCCL P2P not available: %s", e)
        return False
...
class AutoregressiveCUDABackend(InferenceBackend):
    """Extreme low-level optimized autoregressive backend for CUDA."""
    ...
    def __init__(self, config: BackendConfig):
        ...
        # CUDA graph manual capture was permanently removed (commit 19d979f).
        # The captured graph could not include past_key_values as a static input,
        # so replay produced garbage. torch.compile internally handles graph
        # capture correctly when mode="reduce-overhead" (PyTorch >= 2.14).
        #
        # PagedAttention is opt-in via config (use_paged_attention=True).
        # When enabled, PagedCache is passed as past_key_values to model() —
        # a drop-in DynamicCache replacement requiring zero model code changes.
        # When disabled, the standard HF contiguous cache is used.
        self._use_paged_attention: bool = extra.get("use_paged_attention", False)  # opt-in
        ...
        # ── cudaMallocAsync — enabled when compile doesn't use cudagraph_trees ──
        self._malloc_async_active: bool = False
        self._compile_uses_graphs: bool = False

        # ── FlashAttention-3 — Hopper-optimized WGMMA/TMA kernels ──
        self._fa3_func: Any = None  # set in load() if FA3 + Hopper detected
        ...
        if self._safe_mode:
            ...
            logger.info(
                "SAFE MODE active — paged attention, "
                "quantized weights, INT8 KV-cache, "
    ...
    def load(self) -> None:
        ...
        # ── 1. Memory: cudaMallocAsync (deferred — must run AFTER compile) ──
        # cudaMallocAsync is incompatible with torch.compile's cudagraph_trees.
        # We check _compile_uses_graphs AFTER _apply_extreme_compile.
        if self.backend_name == "cuda":
            _enable_nccl_p2p()

        # ── 2. Devices ──
        if self.backend_name == "cuda":
            # Honour explicit device from device_info (DataParallelBackend sets it).
            # Otherwise count from 0 through num_devices.
        ...
        if self.backend_name == "cuda" and self.use_flash_attention:
            try:
                ...
                # ── FlashAttention-3 for Hopper (SM90) ──
                # FA3 uses SM90-specific features (WGMMA async copies, TMA
                # for KV-cache loads) that deliver 1.5-2× attention throughput
                # over FA2 on H200.  It requires the flash-attn package v3+.
                # When available on Hopper, FA3 replaces the PyTorch SDPA
                # backend for both prefill (causal) and decode (non-causal
                # with KV-cache) paths.
                try:
                    import flash_attn
                    _fa3 = hasattr(flash_attn, "__version__") and flash_attn.__version__.startswith("3")
                except ImportError:
                    _fa3 = False
                ...
                if _fa3:
                    ...
                    try:
                        ...
                        pass
                    _is_hopper = any(
                        tag in _gpu_name for tag in ("H100", "H200", "H800", "GH200")
                    )
                    if _is_hopper:
                        # Configure FA3 as the preferred SDPA backend.
                        # PyTorch SDPA auto-dispatches to FA3 when the
                        # flash-attn v3 package provides the Hopper kernel.
                        if hasattr(torch.backends.cuda, "enable_flash_attention"):
                        ...
                        if hasattr(flash_attn, "flash_attn_func"):
                            ...
                            logger.info(
                                "FlashAttention-3 ACTIVE on %s — WGMMA/TMA "
                                "Hopper kernels enabled (∼1.5-2× attention throughput)",
                                _gpu_name,
                            )
                        else:
                            self._fa3_func = None
                            logger.info(
                                "FlashAttention-3 detected on %s but "
                                "flash_attn_func not available (using SDPA dispatch)",
                                _gpu_name,
                            )
                    else:
                        self._fa3_func = None
                        logger.info(
                            "FlashAttention-3 package detected but GPU '%s' "
                            "is not Hopper (SM90).  Using standard Flash SDPA.",
                            _gpu_name,
                        )
                else:
                    self._fa3_func = None
        ...
        # Move model to target device if it landed elsewhere.
        _target_device = self.devices[0] if self.devices else None
        if _target_device is not None and self.backend_name in ("cuda", "mps", "cpu"):
            _first_param = next(self.model.parameters(), None)
            if _first_param is not None and _first_param.device != _target_device:
                logger.info(
                    "%s: model on %s — moving to %s",
        ...
        # ── 8. torch.compile ──
        # Skip compile for paths using PagedCache (PagedCache.update() causes
        # Dynamo graph breaks — scaled_dot_product_attention with invalid shapes).
        # Per TPS Acceleration Plan Tier 3/4 trade-off: PagedAttention (1.8×
        # via larger batch sizes) >> torch.compile (<5% on 4B). Prioritize paged.
        _skip_compile = (
            getattr(self.config, 'use_continuous_batching', False) or
            self.config.extra.get("use_continuous_batching", False) or
            self._use_paged_attention  # Tier 3: PagedCache not Dynamo-traceable
        )
        if self.use_torch_compile and self.backend_name != "cpu" and not self._safe_mode:
            if _skip_compile:
                if self._use_paged_attention:
                    logger.info(
                        "torch.compile skipped — PagedAttention active (PagedCache "
                        "not Dynamo-traceable). PagedAttention 1.8× >> compile <5%."
                    )
                else:
                    logger.info("torch.compile skipped — CB (PagedCache not Dynamo-traceable)")
                self.use_torch_compile = False
            else:
                self._apply_extreme_compile()

        # ── 8b. cudaMallocAsync — must run AFTER compile (needs _compile_uses_graphs) ──
        if self.backend_name == "cuda":
            if not self._compile_uses_graphs:
                self._malloc_async_active = _enable_cuda_malloc_async()
            else:
                logger.info(
                    "cudaMallocAsync disabled — torch.compile mode='reduce-overhead' "
                    "uses cudagraph_trees (incompatible with stream-ordered allocation)"
                )

        # ── 9. PagedAttention pool ──
    ...
    def _report_capabilities(self) -> None:
        ...
        reg.register(CapabilityEntry(
            ...
            reason=f"mode={'reduce-overhead' if self._compile_uses_graphs else 'default'}"
        ...
        reg.register(CapabilityEntry(
            ...
            state=ActivationState.ACTIVE if self._malloc_async_active
                  else ActivationState.INERT,
            reason="Stream-ordered pool allocator" if self._malloc_async_active
                  else "Not enabled (safe mode, non-CUDA, or cudagraph_trees conflict)",
            phase="hot_path",
        ))
        reg.register(CapabilityEntry(
            feature_id="speculative_decode", display_name="Speculative Decoding",
            state=ActivationState.ACTIVE if self._spec_decoder is not None
                  else ActivationState.INERT,
            reason="Self-speculative (early-layer draft) or draft-model mode"
                  if self._spec_decoder is not None
                  else "Not requested or not applicable",
            phase="hot_path",
        ))
        ...
        reg.register(CapabilityEntry(
            ...
            state=ActivationState.ACTIVE if (self._use_paged_attention and self._paged_kv is not None)
                  else ActivationState.INERT,
            reason="Wired: prefill → paged blocks → PagedCache → model forward"
                   if (self._use_paged_attention and self._paged_kv is not None)
                   else "Opt-in via --paged-attention (CUDA only)",
            phase="hot_path",
        ))
        reg.register(CapabilityEntry(
    ...
    def _load_standard_model(self, dtype: torch.dtype) -> None:
        ...
        if self.backend_name == "cuda":
            ...
            if _single_gpu or n_devs == 1:
                if '_est_bytes' not in locals():
                    _est_bytes = 0.0
                    single_gpu_mem = torch.cuda.get_device_properties(self.devices[0]).total_memory
    ...
    def _apply_extreme_compile(self) -> None:
        ...
        try:
            ...
            self.model = compiled
            self._compile_uses_graphs = (opts.get("mode") == "reduce-overhead")
            logger.info("torch.compile applied (mode=%s, cudagraph_trees=%s)",
                        opts.get("mode"), self._compile_uses_graphs)
        except Exception as e:
            logger.warning(
    ...
    def _init_paged_attention(self) -> None:
        ...
        NOTE (2026-06): This method is opt-in via ``--paged-attention``
        (sets ``use_paged_attention=True`` in backend config extra dict,
        read at line 539).  When enabled, the PagedKVCache replaces HF's
        default contiguous KV-cache via the PagedCache DynamicCache shim.
        When disabled (default), the standard HF past_key_values tuple is used.
        """
        try:
    ...
    def warmup(self, batches: int = 20) -> None:
        ...
        logger.info("AR warmup (extreme): %d batches...", batches)
        ...
        # ── PagedAttention: skip large-batch warmup ──
        # When PagedAttention is active, Phase 2 prefill at full batch size
        # creates massive contiguous DynamicCache allocations (bs=1024 × 120
        # tokens × 36 layers = ~33 GB K+V per forward). The allocator thrashes.
        # PagedAttention operates per-sequence in _extreme_decode — a large
        # contiguous prefill here does NOT warm the paged decode path. Skip it.
        warmup_bs = getattr(self, '_configured_batch_size', 1)
        if self.backend_name == "mps":
            warmup_bs = max(warmup_bs, 1)
            n_iters = 2
        elif self._use_paged_attention and self._paged_kv is not None:
            # PagedAttention: skip large contiguous prefill — it doesn't warm
            # the paged decode path and allocates massive DynamicCache tensors.
            logger.info(
                "AR warmup Phase 2: SKIPPED — PagedAttention active "
                "(contiguous prefill doesn't warm paged decode path)"
            )
            n_iters = 0  # skip entirely
        ...
        if n_iters > 0:
            ...
            del ids_single  # no tensor allocated, just free the list
        ...
        if self.backend_name == "cuda":
            ...
            try:
                ...
                pass  # may fail if a capture is still in progress
    ...
    def translate_batch(self, batch: Any) -> BatchGenerationOutput:
        ...
        If CUDA is available:
          Prefill (1 forward pass) → Decode loop (standard forwards × N tokens)
          torch.compile provides internal CUDA graphs via reduce-overhead mode.
        Otherwise:
        ...
        # PERF: speculative processes sequences one-at-a-time in Python.
        # At bs > 16 the per-sequence overhead dominates — standard batched
        # decode with vectorized EOS is orders of magnitude faster.
        bs_actual = batch.input_ids.shape[0] if hasattr(batch, 'input_ids') else 1
        if self._spec_decoder is not None and bs_actual <= 16:
            return self._spec_decoder.translate_batch(batch, self)
        elif self._spec_decoder is not None and bs_actual > 16:
            pass  # skip speculative, fall through to _extreme_decode
    ...
    def _extreme_decode(
        ...
        with torch.no_grad(), self._fp8_context():
            ...
            with torch.cuda.nvtx.range("prefill"):
                ...
                # ── Wire PagedAttention: write prefill KV into paged blocks ──
                # PagedCache is a drop-in DynamicCache replacement — the model's
                # attention layers call past_key_value.update() which routes to
                # PagedLayer.update(), writing into blocks and returning assembled KV.
                if self._paged_kv is not None:
                    from benchmark.inference.paged_attention import PagedCache
                    prompt_len = input_ids.shape[1]
                    for seq_idx in range(bs):
                        self._paged_kv.allocate(seq_idx, prompt_len)
                        for layer_idx in range(len(past_kv)):
                            k_tensor = past_kv[layer_idx][0][seq_idx:seq_idx+1]
                            v_tensor = past_kv[layer_idx][1][seq_idx:seq_idx+1]
                    ...
                    # Free the contiguous DynamicCache tensors — they've been
                    # copied into paged blocks and would otherwise hold ~33 GB
                    # of GPU memory alive (bs=512, prompt=120, 36 layers).
                    _old_kv = past_kv
                    past_kv = PagedCache(self._paged_kv, seq_ids=list(range(bs)))
                    del _old_kv, prefill_out
                    torch.cuda.empty_cache()
                    # Re-fetch prefill_out for timing below (logits no longer needed)
                    prefill_out = None

            ev_prefill_end.record()

            # ── DECODE LOOP (vectorized EOS — Tier 2 TPS acceleration) ────
            # GPU-side tensor ops replace per-sequence Python loop + .item() sync.
            # KEY INVARIANT: never feed PAD (0) to the model. Finished sequences
            # continue receiving their last real token — the model generates
            # more tokens but we ignore them (masked at recording time).
            ...
            for step in range(max_new):
                ...
                # Record tokens ONLY for unfinished sequences. Finished ones
                # keep receiving real tokens (no PAD pollution) but we ignore.
                is_eos = (next_tokens == eos_id) | (next_tokens == eot_id)
                generated[:, step] = torch.where(
                    unfinished, next_tokens, torch.zeros_like(next_tokens),
                )
                ...
                # Feed real tokens to ALL sequences — finished ones re-process
                # their last token (harmless: deterministic greedy decode)
                next_input = next_tokens.unsqueeze(-1)

            # Convert GPU tensors → list[list[int]] once at the end
            generated_ids: list[list[int]] = [
                generated[i, :num_generated[i].item()].tolist() for i in range(bs)
            ]
    ...
    # ── SmoothQuant calibration (default on CUDA, skip via TR_SKIP_SMOOTHQUANT=1) ──
    ...
    def _calibrate_smoothquant(self) -> None:
        ...
        # Prefer the configured input paths from the pipeline; fall back
        # to the project-root data directory only if no config is available.
        cal_glob = getattr(self, '_calibration_input_glob', None)
        if cal_glob is None:
            from pathlib import Path as _Path
            _project_data = _Path(__file__).resolve().parent.parent.parent.parent / "data" / "input" / "*.jsonl.gz"
            cal_glob = str(_project_data)
        loader = JSONLLoader([cal_glob], shuffle=False)
        cal_texts: list[str] = []
        ...
        if not cal_texts:
            logger.warning(
                "SmoothQuant calibration: no calibration texts found at %s. "
                "FP8 weights will be uncalibrated — quality may degrade. "
                "Set TR_SKIP_SMOOTHQUANT=1 to suppress this warning.",
                cal_glob,
            )
