...
batches, tokens = 0, 0; t0 = time.monotonic()
while time.monotonic() - t0 < 60:
    ...
    batches += 1; tokens += result.output_tokens_total
elapsed = time.monotonic() - t0
tps = tokens/elapsed if elapsed > 0 else 0
pipeline.stop_prefetch()
print(f'NGC: {batches} b, {tokens} tok, {elapsed:.1f}s, {tps:.0f} tok/s ({tps/816:.2f}x vs pip BF16)')
